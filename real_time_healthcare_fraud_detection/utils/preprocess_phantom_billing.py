...
def preprocess_data(df, is_training=False, label_column=None, return_meta=False, visualize=False):
    ...
    # === Parse Dates and Length of Stay ===
    ...
    df["High Billed"] = (df["Amount Billed"] > 250000).astype(int)
    df["Invalid Diagnosis Combo"] = (
        (df["Diagnosis"].str.lower().str.contains("pregnancy", na=False)) & (df["Gender"] == 0)
    ).astype(int)
    ...
    if visualize and "Fraud Type" in df.columns:
        ...
        for feature in ["Amount Billed", "Length of Stay", "Billing Per Day"]:
            ...
            sns.histplot(data=df, x=feature, hue="Fraud Type", bins=50, kde=True, palette="Set2", element="step")
            plt.title(f"{feature} Distribution by Fraud Type")
            plt.xlabel(feature)
            plt.ylabel("Frequency")
            plt.tight_layout()
            plot_path = f"plots/{feature.replace(' ', '_').lower()}_by_fraud_type.png"
